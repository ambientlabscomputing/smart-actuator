// src/grpc.rs — gRPC ActuatorService server.
//
// Mirrors actuator-sim/src/grpc.rs exactly: the same `Servicer` wrapping
// `Arc<dyn Service>`, the same RPC implementations, the same refusal-code
// mapping. The Sidecar cannot distinguish this from a running actuator-sim
// instance — that is the architecture test for J6.
//
// Phase 1 spike: `tonic::transport::Server` on xtensa-esp32-espidf requires
// hyper + h2 to compile for ESP-IDF's lwIP stack. Run `make firmware-check`
// to surface any failures early. Fallback plan: prost + raw TCP (see RFD-10).
...
// Identical to actuator-sim's mapping. If this diverges, it is a bug.
...
impl ActuatorService for Servicer {
    ...
    async fn get_clock(
        ...
        // Real hardware has no sim clock; return 0.0.
        Ok(Response::new(ClockResponse { sim_time_s: 0.0 }))
    }
}

// ── Server entry point ────────────────────────────────────────────────────────

/// Bind and serve the ActuatorService on `0.0.0.0:port`.
/// Runs until `shutdown` resolves (or the program is killed).
pub async fn serve(port: u16, service: Arc<dyn Service>) -> anyhow::Result<()> {
    let addr = format!("0.0.0.0:{port}").parse()?;
    let servicer = Servicer { inner: service };

    info!("gRPC ActuatorService listening on {}", addr);
